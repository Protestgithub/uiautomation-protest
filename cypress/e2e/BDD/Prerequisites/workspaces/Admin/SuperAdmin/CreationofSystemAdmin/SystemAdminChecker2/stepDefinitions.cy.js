...
Given('Login into Mobiquity Portal as System admin Checker2 Created by Master', function () {
  cy.launchURL(Cypress.env('Adminurl'))
  cy.readFile(Sysfilelogin).then((data) => {
    cy.login(data.LoginId2, this.data1.DefaultPassword)
    cy.login1(this.data1.BAPassword)
    cy.Passwordchange(this.data1.UserCreationSuccessMessage)
    pageLogin.getloginbtn1().eq(0).click()
    cy.wait(2000)
    cy.login(data.LoginId2, this.data1.BAPassword)
...
Given('x.4 Login into Mobiquity Portal as System admin Checker2 Created by Master', function () {
  cy.launchURL(Cypress.env('Adminurl'))
  cy.readFile(Sysfilelogin).then((data) => {
    cy.login(data.LoginId2, this.data1.DefaultPassword)
    cy.login1(this.data1.BAPassword)
